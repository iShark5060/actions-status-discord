...
export function fitEmbed(embed: any): any {
  if (embed.title) {
    ...
    if (titleLen > constants.MAX_EMBED_TITLE_LENGTH) {
      logWarning(
        `embed title must be shorter than ${constants.MAX_EMBED_TITLE_LENGTH}, got ${titleLen}\n    ${embed.title}`,
      );
  ...
  if (embed.description) {
    ...
    if (descLen > constants.MAX_EMBED_DESCRIPTION_LENGTH) {
      logWarning(
        `embed description must be shorter than ${constants.MAX_EMBED_DESCRIPTION_LENGTH}, got ${descLen}\n    ${embed.description}`,
      );
  ...
  if (embed.fields) {
    for (const field of embed.fields) {
      ...
      if (nameLen > constants.MAX_EMBED_FIELD_NAME_LENGTH) {
        logWarning(
          `embed field name must be shorter than ${constants.MAX_EMBED_FIELD_NAME_LENGTH}, got ${nameLen}\n    ${field.name}`,
        );
        field.name = truncStr(field.name, constants.MAX_EMBED_FIELD_NAME_LENGTH);
      }
      if (valueLen > constants.MAX_EMBED_FIELD_VALUE_LENGTH) {
        logWarning(
          `embed field value must be shorter than ${constants.MAX_EMBED_FIELD_VALUE_LENGTH}, got ${valueLen}\n    ${field.value}`,
        );
...
export function fitContent(content: string): string {
  ...
  if (contentLen > constants.MAX_WEBHOOK_CONTENT_LENGTH) {
    logWarning(
      `content field must be shorter than ${constants.MAX_WEBHOOK_CONTENT_LENGTH}, got ${contentLen}\n    ${content}`,
    );
